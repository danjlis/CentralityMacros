...
void Fun4All_CentralityReco(const int runnumber, const int rollover = 0)
{
  gSystem->Load("libg4dst");
  gSystem->Load("libcentrality_io");
  ...
  const char *hist_outfile = "./outhist1.root";//Form("/gpfs02/sphenix/user/dlis/Projects/centrality/output/run%d/centrality_reco_hist_%s_%s.root", runnumber, rstr.str().c_str(), ostr.str().c_str());
  const char *tree_outfile = "./outtree1.root";//Form("/gpfs02/sphenix/user/dlis/Projects/centrality/output/run%d/centrality_reco_tree_%s_%s.root", runnumber, rstr.str().c_str(), ostr.str().c_str());
  ...
  CentralityReco *cr = new CentralityReco("CentralityReco", hist_outfile, tree_outfile);
  cr->SetOperationMode(CentralityReco::OperationMode::Performance);
  cr->SetVerbosity(0);
